...
| `save_confirm` | `true` или объект | Спросить подтверждение перед сохранением, если поле меняли |

### save_confirm

Формат тот же, что у `confirm` в кнопках: `true` либо
`{title, message, okTitle, okVariant, cancelTitle, cancelVariant}`.

```sql
mark_code @{
    save_confirm = {
        title: 'Подтвердите отправку чека',
        message: 'Будет пробит фискальный чек, код выйдет из оборота. Продолжить?',
        okTitle: 'Пробить чек',
        okVariant: 'danger'
    }
}
...
Поведение: изменение такого поля не отправляется сразу — пока подтверждение не получено,
автосохранение блокируется (иначе диалог всплыл бы сам через несколько секунд после ввода).
Диалог показывается один раз перед отправкой транзакции; при отказе изменения остаются
в форме несохранёнными. Если за одно сохранение изменено несколько таких полей,
показывается одно окно со списком сообщений.

Проверяется тестом `e2e/save-confirm.spec.js` (нужен `E2E_SAVE_CONFIRM_URL`).
